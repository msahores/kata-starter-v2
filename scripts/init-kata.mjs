...
function generateKataMd(title, description) {
  ...
  lines.push('## Rules')
  lines.push('')
  lines.push('<!-- Describe the rules and constraints of the kata -->')
  lines.push('')
  lines.push('## Examples')
  lines.push('')
  lines.push('<!-- Add input/output examples here -->')
  lines.push('<!-- e.g.: `f(1)` → `1`, `f(5)` → `"fizz"` -->')
  lines.push('')
  lines.push('## Edge Cases')
  lines.push('')
  lines.push('<!-- List edge cases to consider -->')
  lines.push('')
  return lines.join('\n')
}
